...
struct PSCSVParser {
    func parseCSV(contentsOfURL: NSURL, encoding: String.Encoding, completion: @escaping (Result<[PSProduct]?, Error>) -> Void) {
        ...
        do {
            ...
            for line in lines {
                var values:[String] = []
                let item = parse(by: line, encoding: .utf8)
                items.append(item)
//                if line != "" {
...
//                }
    ...
    func parse(by line: String, encoding: String.Encoding) -> PSProduct {
        let delimiter = ","
        var item = PSProduct(productId: "", title: "", listPrice: 0, salesPrice: 0, color: "", size: "")
        // Load the CSV file and parse it
        ...
        if line != "" {
            ...
            // Put the values into the tuple and add it to the items array
            item = PSProduct(productId: values[0], title: values[1], listPrice: Double(values[2]) ?? 0, salesPrice: Double(values[3]) ?? 0, color: values[4], size: values[5])
